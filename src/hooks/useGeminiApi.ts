...
- Return ONLY valid JSON`;
    ...
    // Delay before Phase 1B to avoid rate limiting (12s ensures 60s cycle with 5 keys)
...
- Return ONLY valid JSON`;
    ...
    // Delay before scene extraction to avoid rate limiting (12s ensures 60s cycle with 5 keys)
    await new Promise(resolve => setTimeout(resolve, 12000));
...
- Return ONLY valid JSON`;
        ...
        for (let retry = 0; retry < MAX_RETRIES; retry++) {
          try {
            ...
            if (retry < MAX_RETRIES - 1) {
              console.log(`Retrying chunk ${i + 1} in 12 seconds...`);
              await new Promise(resolve => setTimeout(resolve, 12000));
            }
          }
        }
        ...
        // Delay between chunks to avoid rate limiting (12s ensures 60s cycle with 5 keys)
